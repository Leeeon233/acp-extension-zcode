...
describe("compact() outcome detection", () => {
  it("reports failure on session_compact_failed", async () => {
    ...
    const activities = notifySpy.mock.calls
      .map(([, p]) => p?.update)
      .filter((update) => update?.toolCallId?.startsWith("lody_compact_"));
    expect(activities).toMatchObject([
      { sessionUpdate: "tool_call", status: "in_progress" },
      { sessionUpdate: "tool_call_update", status: "failed" },
    ]);
    expect(activities[1].toolCallId).toBe(activities[0].toolCallId);
  });

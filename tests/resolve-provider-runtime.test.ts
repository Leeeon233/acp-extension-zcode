...
describe("builtinProviderEnv", () => {
  it("points at ../config/provider for the .app bundle layout (the host's own path)", () => {
    const root = realpathSync(mkdtempSync(path.join(tmpdir(), "zcode-bundle-")));
  ...
  it("follows an extensionless symlink to the bundled entry", () => {
    const root = realpathSync(mkdtempSync(path.join(tmpdir(), "zcode-bundle-")));
    const entry = makeAppBundle(root);
    const bin = path.join(root, "zcode");
    symlinkSync(entry, bin);
    expect(builtinProviderEnv(bin)).toEqual(builtinProviderEnv(entry));
  });
